...
namespace Androids2
{
    public class JobDriver_CarryToRepairGantry : JobDriver
    {
        public const TargetIndex TakeeInd = TargetIndex.A;

        public const TargetIndex IngredientInd = TargetIndex.B;

        public const TargetIndex PodInd = TargetIndex.C;

        public List<Thing> pickedUpIngredients = new List<Thing>();

        public List<ThingCount> thingsToTransfer = new List<ThingCount>();

        public Pawn Takee => (Pawn)job.GetTarget(TargetIndex.A).Thing;

        public CompRepairGantry Pod => job.GetTarget(TargetIndex.C).Thing.TryGetComp<CompRepairGantry>();
        ...
        public override IEnumerable<Toil> MakeNewToils()
        {
            ...
            foreach (Toil item in JobDriver_EnterRepairGantry.CollectIngredientsToilsHelper(TargetIndex.B, pawn, pickedUpIngredients))
            {
                yield return item;
            }
            ...
            yield return JobDriver_EnterRepairGantry.PrepareToEnterToil(TargetIndex.C);
        ...
        public Toil TransferIngredientsAndPrepareCarryDownedPawn()
        {
        ...
        public Toil DropCarryToGrabIngredients()
        {
